 ...
 * @author ckn
 */
public class JoinField<T, R> {
    private final JoinParams params;

    public JoinField(JoinParams params) {
        this.params = params;
    }
    ...
    public <L, K> JoinField<T, L> leftJoin(Class<L> joinClass, SFunction<T, K> mainKey, SFunction<L, K> joinKey) {
        return new JoinCustomer<T>(params).leftJoin(joinClass, mainKey, joinKey);
    }

    public <R, K> JoinField<T, R> rightJoin(Class<R> joinClass, SFunction<T, K> mainKey, SFunction<R, K> joinKey) {
        return new JoinCustomer<T>(params).leftJoin(joinClass, mainKey, joinKey);
    }

    public <I, K> JoinField<T, I> innerJoin(Class<I> joinClass, SFunction<T, K> mainKey, SFunction<I, K> joinKey) {
        return new JoinCustomer<T>(params).leftJoin(joinClass, mainKey, joinKey);
    }
     ...
     * @return
     */
    public JoinField<T, R> lastWhere(String sql,Map<String,Object> parameters) {
        params.lastSQL=sql;
